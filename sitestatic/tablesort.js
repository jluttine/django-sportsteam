...
// STABLE SORTING ALGORITHM (ADDED BY JAAKKO LUTTINEN)
 ...
 * @param {Array} b Second tuple.
 */
TableSort.compare_stringasc = function(a, b) {
  var aLower = a[0].toLowerCase();
  var bLower = b[0].toLowerCase();
  if (aLower == bLower) {
    return 0;
...
TableSort.compare_stringdesc = function(a, b) {
  var aLower = a[0].toLowerCase();
  var bLower = b[0].toLowerCase();
  if (aLower == bLower) {
    return 0;
 ...
 * @param {Array} b Second tuple.
 */
TableSort.compare_numericasc = function(a, b) {
...
TableSort.compare_numericdesc = function(a, b) {
  var aNum = parseFloat(a[0]);
  if (isNaN(aNum)) {
    aNum = -Number.MAX_VALUE;
  }
